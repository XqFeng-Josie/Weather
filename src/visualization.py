...
def visualize_predictions_improved(
    y_test, y_test_pred, test_metrics, variables, model_name, output_dir, 
    data_format="flat", norm_params=None, spatial_coords=None
):
    """
    改进的可视化方案（非重叠样本）
    ...
    for var_idx, var_name in enumerate(variables):
        ...
        selected_samples = np.arange(0, min(50 * stride_samples, n_samples), stride_samples)
        ...
        ax.plot(lead_hours, rmse_per_leadtime, 'o-', linewidth=2, markersize=8, color='darkblue')
        ax.set_xlabel('Lead Time (hours)', fontsize=12)
        ax.set_ylabel('RMSE', fontsize=12)
        ax.set_title(f'{var_name} - RMSE vs Lead Time\n(Non-overlapping samples)', 
                    fontsize=14, fontweight='bold')
        ax.grid(True, alpha=0.3)
        
        # 标注数值
        for i, (hour, rmse) in enumerate(zip(lead_hours, rmse_per_leadtime)):
            ax.text(hour, rmse, f'{rmse:.4f}', 
        ...
        if norm_params is not None and var_name in norm_params['mean']:
            ...
            ax.plot(time_indices, time_series_true_denorm, 'b-', label='True', alpha=0.7, linewidth=1.5)
            ax.plot(time_indices, time_series_pred_denorm, 'r-', label='Pred', alpha=0.7, linewidth=1.5)
            ax.set_xlabel('Time Step Index', fontsize=12)
            ...
            ax.set_title(f'{var_name} - Overall Time Series (Physical Values)', 
                        fontsize=14, fontweight='bold')
            ax.legend(fontsize=12)
            ax.grid(True, alpha=0.3)
            ax.set_xlim(left=0)
        ...
        if data_format == "spatial" and spatial_coords is not None:
            ...
            channel_idx = var_idx if y_test.shape[2] == n_variables else var_idx
            y_true_spatial = y_test[:, :, channel_idx, :, :]  # (samples, lead_times, H, W)
            ...
            if len(lat) != H or len(lon) != W:
                print(f"  警告: 坐标维度不匹配 (lat:{len(lat)} vs H:{H}, lon:{len(lon)} vs W:{W})")
                print(f"  使用默认坐标...")
            ...
            # 标记2个固定点
            ax1.plot(point1_lon, point1_lat, 'ro', markersize=12, 
                    transform=ccrs.PlateCarree(), markeredgecolor='white', markeredgewidth=2)
            ax1.plot(point2_lon, point2_lat, 'go', markersize=12, 
                    transform=ccrs.PlateCarree(), markeredgecolor='white', markeredgewidth=2)
            ...
            ax1.text(point1_lon, point1_lat + 5, f'P1: {val1_true:.1f}', 
                    transform=ccrs.PlateCarree(), fontsize=10, 
                    ha='center', va='bottom', color='red', fontweight='bold',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
            ax1.text(point2_lon, point2_lat + 5, f'P2: {val2_true:.1f}', 
                    transform=ccrs.PlateCarree(), fontsize=10,
            ...
            im2 = ax2.contourf(lon_grid, lat_grid, data_pred, 
                              levels=15, cmap='RdYlBu_r', 
                              vmin=vmin, vmax=vmax,
                              transform=ccrs.PlateCarree())
            ...
            # 标记2个固定点
            ax2.plot(point1_lon, point1_lat, 'ro', markersize=12, 
                    transform=ccrs.PlateCarree(), markeredgecolor='white', markeredgewidth=2)
            ax2.plot(point2_lon, point2_lat, 'go', markersize=12, 
                    transform=ccrs.PlateCarree(), markeredgecolor='white', markeredgewidth=2)
            ...
            ax2.text(point1_lon, point1_lat + 5, f'P1: {val1_pred:.1f}', 
                    transform=ccrs.PlateCarree(), fontsize=10,
                    ha='center', va='bottom', color='red', fontweight='bold',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
            ax2.text(point2_lon, point2_lat + 5, f'P2: {val2_pred:.1f}', 
                    transform=ccrs.PlateCarree(), fontsize=10,
                    ha='center', va='bottom', color='green', fontweight='bold',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
            ...
            plt.close()
            print(f"  ✓ 保存: spatial_comparison_{safe_var_name}.png (True vs Pred 世界地图对比)")
