...
def crps_ensemble(
    observations: np.ndarray, forecasts: np.ndarray, axis: int = 0
) -> np.ndarray:
    """
...
def spread_skill_ratio(
    forecasts: np.ndarray, observations: np.ndarray, axis: int = 0
) -> float:
    """
...
def continuous_ranked_probability_score_summary(
    forecasts: np.ndarray, observations: np.ndarray, variables: list = None
) -> dict:
    """
    计算综合的 CRPS 统计信息
    ...
    # Spread-Skill Ratio
    stats["spread_skill_ratio"] = float(
        spread_skill_ratio(forecasts, observations, axis=0)
    )
...
def probabilistic_evaluation_report(
    ...
    Returns:
        报告字符串
    """
    stats = continuous_ranked_probability_score_summary(
        forecasts, observations, variables
    )
