...
class RAEWrapper:
    ...
    def __init__(
        self,
        encoder_cls: str = "SigLIP2wNorm",
        encoder_config_path: str = "google/siglip2-base-patch16-256",
        ...
        # 如果没有提供encoder_params，根据encoder_cls设置默认值
    ...
    def get_decoder_output_size(self) -> tuple:
        """
        获取decoder输出图像尺寸

        Returns:
            (H, W): decoder输出图像的高度和宽度
        """
        # RAE decoder的输出尺寸由decoder_config.image_size决定
        # 计算公式: decoder_image_size = decoder_patch_size * sqrt(base_patches)
        # 其中 base_patches = (encoder_input_size // encoder_patch_size)^2
        decoder_image_size = self.rae.decoder.config.image_size
        if isinstance(decoder_image_size, (list, tuple)):
            return tuple(decoder_image_size)
        else:
            return (decoder_image_size, decoder_image_size)
