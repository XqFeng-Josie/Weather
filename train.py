...
from src.models.linear_regression import LinearRegressionModel, MultiOutputLinearRegression
...
def main():
    ...
    if data_format == 'spatial':
        ...
        print(f"Channels: {data_info['n_channels']}, Spatial size: {data_info['spatial_size']}")
    ...
    lr = args.lr * 0.1 if (args.model == 'transformer' and len(variables) == 1) else args.lr
    ...
    # 针对不同数据格式的可视化
    if data_format == 'flat':
        visualize_flat_predictions(y_test, y_test_pred, test_metrics, variables, args, output_dir)
    else:
        visualize_spatial_predictions(y_test, y_test_pred, test_metrics, variables, args, output_dir)

    print(f"\n✓ Results saved to {output_dir}")
    print(f"✓ Model saved to {output_dir}/best_model.pth")


def visualize_flat_predictions(y_test, y_test_pred, test_metrics, variables, args, output_dir):
    """可视化展平格式的预测结果（多变量）"""
    import matplotlib.pyplot as plt
    ...
    for var_idx, var_name in enumerate(variables):
        ...
        for i, ax in enumerate(axes.flat):
            ...
            ax.plot(y_test[sample_idx, i, feature_idx], "b-o", label="True", alpha=0.7, markersize=3)
            ax.plot(
                y_test_pred[sample_idx, i, feature_idx], "r-s", label="Pred", alpha=0.7, markersize=3
            )
        ...
        plt.savefig(output_dir / f"predictions_{safe_var_name}.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    if n_variables > 1:
        ...
        for var_idx, var_name in enumerate(variables):
            ...
            # 第一个时间步
            ax = axes[var_idx, 0]
            ax.plot(y_test[sample_idx, 0, feature_idx], "b-o", label="True", alpha=0.7, markersize=3)
            ax.plot(y_test_pred[sample_idx, 0, feature_idx], "r-s", label="Pred", alpha=0.7, markersize=3)
            ...
            ax.plot(y_test[sample_idx, last_t, feature_idx], "b-o", label="True", alpha=0.7, markersize=3)
            ax.plot(y_test_pred[sample_idx, last_t, feature_idx], "r-s", label="Pred", alpha=0.7, markersize=3)
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "predictions_all_variables.png", dpi=300, bbox_inches="tight")
        plt.close()
...
def visualize_spatial_predictions(y_test, y_test_pred, test_metrics, variables, args, output_dir):
    """可视化空间格式的预测结果（多变量）"""
    import matplotlib.pyplot as plt
    ...
    # 为每个变量绘制空间场图
    for channel_idx in range(min(n_channels, n_variables)):
        var_name = variables[channel_idx] if channel_idx < len(variables) else f"Var{channel_idx}"
    ...
    for i, ax in enumerate(axes.flat):
        ...
        # 提取该时间步的空间点
        sample_idx = np.linspace(0, len(y_test) - 1, 50, dtype=int)
        y_true_point = y_test[sample_idx, i, channel_idx, point_h, point_w]
        y_pred_point = y_test_pred[sample_idx, i, channel_idx, point_h, point_w]
