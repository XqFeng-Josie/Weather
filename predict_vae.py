"""
VAE (SD) Latent U-Net预测脚本

使用方法:
    python predict_vae.py --model-dir outputs/vae_latent_unet --time-slice 2020-01-01:2020-12-31
"""
...
def main():
    parser = argparse.ArgumentParser(description="VAE (SD) Latent U-Net预测")
    ...
    # 验证VAE类型
    vae_type = config.get("vae_type", "sd")
    if vae_type != "sd":
        raise ValueError(
            f"此脚本仅支持SD VAE，但配置中vae_type={vae_type}。请使用predict_rae.py或predict_unet.py"
        )
    ...
    vae_model_id = config.get(
        "vae_model_id",
        normalizer_data.get("vae_model_id", "runwayml/stable-diffusion-v1-5"),
    )
    ...
    freeze_encoder = config.get(
        "freeze_encoder",
        normalizer_data.get("freeze_encoder", True),
    )
    freeze_decoder = config.get(
        "freeze_decoder",
        normalizer_data.get("freeze_decoder", True),
    )

    print(f"使用Stable Diffusion VAE: {vae_model_id}")
    if vae_pretrained_path:
        print(f"  预训练权重路径: {vae_pretrained_path}")
    print(f"  Encoder冻结: {freeze_encoder}")
    print(f"  Decoder冻结: {freeze_decoder}")
    ...
    with torch.no_grad():
        for inputs, targets in tqdm(test_loader, desc="预测中(VAE Latent)"):
    ...
    # 需要在计算指标之前进行选择，以便归一化空间和物理空间都使用选中的channels
    ...
    # 归一化空间的指标（如果指定了 --levels，使用选中的 channels）
    print("\n归一化空间的指标:")
