...
def calculate_reconstruction_metrics(original_images: np.ndarray,
...
def create_comparison_visualization(original_images: np.ndarray,
    ...
    fig, axes = plt.subplots(n_methods + 1, n_samples, figsize=(4 * n_samples, 4 * (n_methods + 1)))
    ...
    # 后续行：各个重建结果
    for row, (recon_imgs, label) in enumerate(zip(reconstructed_images_list, labels), start=1):
...
def create_metrics_comparison_table(metrics_list: list, labels: list, output_path: Path):
    """
    创建指标对比表格
    ...
    # 准备表格数据
    table_data = []
    headers = ['Method', 'RMSE (K)', 'MAE (K)', 'PSNR (dB)', 'SSIM', 'Correlation']
    
    for metrics, label in zip(metrics_list, labels):
        if 'original_space' in metrics:
            m = metrics['original_space']
            row = [
                label,
                f"{m.get('rmse', 0):.4f}",
                f"{m.get('mae', 0):.4f}",
    ...
    plt.title('Reconstruction Metrics Comparison', fontsize=14, fontweight='bold', pad=20)
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
...
def main():
    ...
    if args.labels:
        ...
        if len(labels) != len(recon_dirs):
            raise ValueError(f"标签数量 ({len(labels)}) 与重建目录数量 ({len(recon_dirs)}) 不匹配")
    else:
    ...
    # 确保所有图像数量一致
    n_samples = min(len(original_images), min(len(imgs) for imgs in reconstructed_images_list))
